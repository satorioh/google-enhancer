...
function createParentMenu (obj) {
		chrome.contextMenus.create(obj,function() {
...
storage.get(function (response) {
	if (response.siteSearch) {
		createParentMenu(contextMenuParents[0]);
	}
	if (response.filetypeSearch) {
		createParentMenu(contextMenuParents[1]);
		let fileType = ["pdf","doc","xls","ppt","ps","dwf","kml","kmz","rtf","swf"];
		createSubMenu(fileType,"ge.filetype");
	}
	if (response.timeRangeSearch) {
		createParentMenu(contextMenuParents[2]);
		let timeRange = ["any time","past hour","past 24 hour","past week","past month","past year","custom range"];
		createSubMenu(timeRange,"ge.timerange");
	}
});
